...
<Note>
This endpoint reuses the `sessionId` from a completed `/link-complete` call. No additional signature is required - the session already proves ownership of the EOA wallet. Call this immediately after `/link-complete` if allowances are missing.
</Note>

<Warning>
The session is available for **30 minutes** after `/link-complete`. If the session expires, you'll need to restart the linking flow.
</Warning>
...
```typescript TypeScript
// After successful /link-complete, use the same sessionId to set allowances
const response = await fetch('https://api.domeapi.io/v1/polymarket/link-set-allowances', {
  ...
  body: JSON.stringify({
    ...
    id: 'req-3',
    params: {
      sessionId: sessionId, // Same sessionId from /link-prepare
    },
  }),
});
...
# After successful /link-complete, use the same session_id
response = requests.post(
    ...
    json={
        ...
        'id': 'req-3',
        'params': {
            'sessionId': session_id,  # Same session_id from /link-prepare
        },
    },
)
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `sessionId` | `string` | Yes | - | Session ID from a completed `/link-complete` call |
| `chainId` | `number` | No | `137` | Chain ID (137 = Polygon mainnet) |
...
- Completed sessions are kept for **30 minutes** (for `/link-set-allowances`)
- Each session can only be completed **once** via `/link-complete`
- After successful `/link-complete`, the session remains available for `/link-set-allowances`
