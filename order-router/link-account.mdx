...
These endpoints are in beta and still being tested. For most use cases, we strongly recommend using the [SDK's `linkUser()` method](/order-router/index#3-link-user-one-time-setup) instead, which handles the complex signing logic automatically.

Only use these direct endpoints if you have specific requirements like TEE/enclave environments, mobile on-device signing, or custom signing workflows where the SDK cannot be used.
</Warning>
